        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <div class="flex flex-col gap-10 px-5 mt-5">
                <div class="breadcrumb flex items-center gap-[30px]">
                    ...
                    <a href="" class="text-[#7F8190] last:text-[#0A090B] last:font-semibold">Manage Status</a>
                    <span class="text-[#7F8190] last:text-[#0A090B]">/</span>
                    <a href="#" class="text-[#7F8190] last:text-[#0A090B] last:font-semibold ">New Status</a>
                </div>
            </div>
            <div class="header flex flex-col gap-1 px-5 mt-5">
                <h1 class="font-extrabold text-[30px] leading-[45px]">New Status</h1>
            ...
            <form method="POST" enctype="multipart/form-data" action="{{ route('dashboard.approval.administration.update', $status->people_status_id) }}" class="flex flex-col gap-[30px] w-[500px] mx-[70px] mt-10" id="updateForm">
                ...
                <div class="group/status flex flex-col gap-[10px]">
                    <p class="font-semibold">Status Document Clearance</p>
                    <div class="peer flex items-center p-[12px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        <div class="mr-[10px] w-6 h-6 flex items-center justify-center overflow-hidden">
                            <img src="{{asset('images/icons/security-user.svg')}}" class="w-full h-full object-contain" alt="icon">
                        </div>
                        <select id="status" class="pl-1 font-semibold focus:outline-none w-full text-[#0A090B] invalid:text-[#7F8190] invalid:font-normal appearance-none bg-[url('{{asset('images/icons/arrow-down.svg')}}')] bg-no-repeat bg-right" name="status_docclear" required>
                            <option value="{{ $status->status_docclear }}" selected>{{ $status->status_docclear }}</option>
                            <option value="In Process" class="font-semibold">In Process</option>
                            <option value="Passed" class="font-semibold">Passed</option>
                            <option value="Not Passed" class="font-semibold">Not Passed</option>
                        </select>
                ...
                <div class="group/status flex flex-col gap-[10px]">
                    <p class="font-semibold">Status OJE</p>
                    <div class="peer flex items-center p-[12px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        <div class="mr-[10px] w-6 h-6 flex items-center justify-center overflow-hidden">
                            <img src="{{asset('images/icons/security-user.svg')}}" class="w-full h-full object-contain" alt="icon">
                        </div>
                        <select id="status" class="pl-1 font-semibold focus:outline-none w-full text-[#0A090B] invalid:text-[#7F8190] invalid:font-normal appearance-none bg-[url('{{asset('images/icons/arrow-down.svg')}}')] bg-no-repeat bg-right" name="status_oje" required>
                            <option value="{{ $status->status_oje }}" selected>{{ $status->status_oje }}</option>
                            <option value="In Process" class="font-semibold">In Process</option>
                            <option value="Passed" class="font-semibold">Passed</option>
                            <option value="Not Passed" class="font-semibold">Not Passed</option>
                        </select>
                ...
                <div class="group/status flex flex-col gap-[10px]">
                    <p class="font-semibold">Status Onboarding</p>
                    <div class="peer flex items-center p-[12px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        <div class="mr-[10px] w-6 h-6 flex items-center justify-center overflow-hidden">
                            <img src="{{asset('images/icons/security-user.svg')}}" class="w-full h-full object-contain" alt="icon">
                        </div>
                        <select id="status" class="pl-1 font-semibold focus:outline-none w-full text-[#0A090B] invalid:text-[#7F8190] invalid:font-normal appearance-none bg-[url('{{asset('images/icons/arrow-down.svg')}}')] bg-no-repeat bg-right" name="status_onboarding" required>
                            <option value="{{ $status->status_onboarding }}" selected>{{ $status->status_onboarding }}</option>
                            <option value="In Process" class="font-semibold">In Process</option>
                            <option value="Passed" class="font-semibold">Passed</option>
                            <option value="Not Passed" class="font-semibold">Not Passed</option>
                        </select>
                ...
                <div class="flex flex-col gap-[10px]">
                    ...
                    <div class="flex items-center w-[500px] h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        ...
                        <input value="{{ $status->join_date }}" type="date" class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full outline-none" placeholder="Write your better course name" name="join_date" required>
                    </div>
                </div>
                <div class="flex items-center gap-5">
                    <button type="button" id="saveButton" class="w-full h-[52px] p-[14px_20px] bg-[#6436F1] rounded-full font-bold text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Save Status</button>
                </div>
            </form>
        </div>
    </section>
