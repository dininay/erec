            ...
            <div class="flex flex-col px-5 mt-5">
                <div class="w-full flex justify-between items-center">
                    <div class="flex flex-col gap-1">
                        <p class="font-extrabold text-[30px] leading-[45px]">Manage Job</p>
                        <p class="text-[#7F8190]">Provide high quality for best students</p>
                    </div>
                    <a href="{{ route('dashboard.job.create') }}" class="h-[52px] p-[14px_20px] bg-[#6436F1] rounded-full font-bold text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D]">Add New Job</a>
                </div>
            </div>
            <div class="course-list-container flex flex-col px-5 mt-[30px] gap-[30px]">
                <div class="course-list-header flex flex-nowrap justify-between pb-4 pr-10 border-b border-[#EEEEEE]">
                    <div class="flex shrink-0 w-[150px] items-center justify-center">
                        <p class="text-[#7F8190]">No</p>
                    </div>
                    <div class="flex shrink-0 w-[150px] items-center justify-center">
                    ...
                    <div class="flex justify-center shrink-0 w-[150px] items-center justify-center">
                        <p class="text-[#7F8190]">Date Created</p>
                    </div>
                    <div class="flex justify-center shrink-0 w-[120px] items-center justify-center">
                ...
                @forelse($jobs as $job)
                    <div class="list-items flex flex-nowrap justify-between pr-10">
                        <div class="flex shrink-0 w-[150px] items-center justify-center">
                            <p class="font-bold text-lg">{{ $job->reg_id }}</p>
                        </div>
                        <div class="flex shrink-0 w-[150px] items-center justify-center">
                        ...
                        <div class="flex shrink-0 w-[120px] items-center">
                            <div class="relative h-[41px]">
                                <div class="menu-dropdown w-[120px] max-h-[41px] overflow-hidden absolute top-0 p-[10px_16px] bg-white flex flex-col gap-3 border border-[#EEEEEE] transition-all duration-300 hover:shadow-[0_10px_16px_0_#0A090B0D] rounded-[18px]">
                                    ...
                                    <a href="{{ route('dashboard.job.show', $job) }}" class="flex items-center justify-between font-bold text-sm w-full">
                                        Manage
                                    </a>
                                    <a href="{{ route('dashboard.job.edit', $job) }}" class="flex items-center justify-between font-bold text-sm w-full">
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('dashboard.job.destroy', $job) }}" class="">
                                        @csrf
                                        @method('DELETE')
